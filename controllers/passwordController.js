...
const { validateChangePassword } = require("../validation/userValidator");
...
module.exports.sendForgotPasswordLink = asyncHandler(async (req, res) => {
  ...
  const link = `${baseURL}/password/reset-password/${user._id}/${token}`;
  console.log(link);
...
module.exports.resetThePassword = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    res.status(200).render("alert", {
      alertTheme: "success",
      message: "You successfully changed your password... please log in"
    });
  } catch (error) {
    console.log(error);
